...
class SteganographyApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Стеганография с использованием интерполяции")
    ...
    def load_image(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Выберите изображение", "", "Image Files (*.bmp *.jpg *.jpeg *.png)")
        if file_path:
            try:
    ...
    def hide_data(self):
        ...
        stego_array = self.interpolation_method(img_array, message_bits)
    ...
    def interpolation_method(self, img_array, message_bits):
        """Реализация метода сокрытия данных с использованием интерполяции"""
